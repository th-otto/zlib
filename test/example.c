...
static void test_dict_deflate(Byte *compr, uLong comprLen)
{
	...
	err = deflateSetDictionary(&c_stream, (const Bytef *) dictionary, (z_int_t) sizeof(dictionary));
...
static void test_dict_inflate(Byte *compr, uLong comprLen, Byte *uncompr, uLong uncomprLen)
{
	...
	for (;;)
	{
		...
		if (err == Z_NEED_DICT)
		{
			...
			err = inflateSetDictionary(&d_stream, (const Bytef *) dictionary, (z_int_t) sizeof(dictionary));
		}
