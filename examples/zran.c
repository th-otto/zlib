    ...
    off_t in, off_t out, unsigned left, unsigned char *window)
{
    ...
    if (index == NULL) {
        ...
        index->mode <<= 1;
        next = realloc(index->list, sizeof(point_t) * index->mode);
        if (next == NULL) {
    ...
    next = (point_t *)(index->list) + index->have++;
    if (index->have < 0) {
        /* Overflowed the int! */
        deflate_index_free(index);
        return NULL;
...
{
    z_stream strm;              /* inflate engine (gets fired up later) */
    unsigned char buf[CHUNK];   /* input buffer */
    unsigned char win[WINSIZE]; /* output sliding window */
    ...
    index->mode = mode;
    index->length = totout;
    list = realloc(index->list, sizeof(point_t) * index->have);
    if (list == NULL) {
...
static void append_bits(unsigned value, int bits,
...
ptrdiff_t deflate_index_extract(FILE *in, struct deflate_index *index,
    ...
    ret = fseeko(in, point->in - (point->bits ? 1 : 0), SEEK_SET);
    if (ret == -1)
        return Z_ERRNO;
    ch = 0;
    ...
    left = len;          /* number of bytes left to read after offset */
    do {
        unsigned got;
...
int main(int argc, char **argv) {
    ...
    got = deflate_index_extract(in, index, offset, buf, LEN);
    if (got < 0)
        fprintf(stderr, "zran: extraction failed: %s error\n",
                got == Z_MEM_ERROR ? "out of memory" : "input corrupted");
    else {
        fwrite(buf, 1, got, stdout);
        fprintf(stderr, "zran: extracted %ld bytes at %ld\n", got, (long) offset);
    }
